...
| `Z sub-range: N of M tiles register on a Z sub-range` | The sample occupies a slab and the rest of the stack is medium, so registration runs on the slab. **This does not change the measured shift** — phase correlation locates the peak from structure and ignores flat regions. It stops empty planes diluting the rank-correlation quality score, and reads far fewer planes. | Informational. Measured: content at 25/10/5% of the stack gives seam quality 0.72/0.52/0.43 uncropped and 0.99/0.91/0.76 cropped, with the shift exactly right in every case. At 5% the uncropped seam fails a 0.4 threshold for a reason unrelated to its alignment. Disable with `--no-z-content-crop` if you suspect the band is being found wrongly. |
| Many `below_quality` seams AND the stacks are deep with a thin sample | Quality is a rank correlation over the **whole** overlap, so a sample occupying a few percent of Z drags it toward the noise floor. | Check whether the Z sub-range crop fired (`Z sub-range:` in the log). If it did not, the band was not distinct enough to find — the seams may be genuinely unregisterable, or the sample may span the whole stack. |
