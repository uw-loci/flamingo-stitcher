...
class StitchingPipeline:
    ...
    def _verify_illumination_geometry(self, tiles) -> None:
        ...
        raise ValueError(
            ...
            f"To fix it: open the Options tab, press 'Measure from data…' next "
            f"to 'Illumination side lighting the LOW end of the frame' and "
            f"apply what it reports, or set it to {result.low_side} by hand "
            f"(it is {configured} now). If you believe {configured} is right, "
            f"set illumination.verify: false in the stitching config to run "
